...
growingseason <- train_data %>%
  dplyr::select(-loc) %>%
  dplyr::select(c(srad_mean_growingseason:prcp_sum_growingseason), str_emmean, state, region, year)
...
quadramonths <- train_data %>%
  dplyr::select(c(srad_mean_April_May_June_July:prcp_sum_August_September_October_November), str_emmean, state, region, year)
...
bimonths <- train_data %>%
  dplyr::select(c(srad_mean_April_May:prcp_sum_October_November), str_emmean, state, region, year)
